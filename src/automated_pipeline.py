...
class AutomatedPipeline:
    ...
    def __init__(self, config=None):
        ...
        # Store custom date if provided
        self.custom_date = self.config.get('custom_date', None)
    ...
    def get_current_timestamp(self):
        """
        Get current timestamp, using custom date if provided
        """
        if self.custom_date:
            return self.custom_date
        return datetime.now()

    def prepare_data_with_custom_date(self, merged_df):
        """
        Prepare data with custom date-based splitting
        """
        print(f"📅 Preparing data with custom date: {self.custom_date.strftime('%Y-%m-%d')}")
        
        # Ensure Date column is datetime
        merged_df['Date'] = pd.to_datetime(merged_df['Date'])
        
        # Calculate split dates based on custom date
        ...
        train_end_date = merged_df['Date'].min() + pd.Timedelta(days=train_days)
        val_end_date = train_end_date + pd.Timedelta(days=val_days)
        
        print(f"📊 Data range: {merged_df['Date'].min().strftime('%Y-%m-%d')} to {merged_df['Date'].max().strftime('%Y-%m-%d')}")
        print(f"📊 Train until: {train_end_date.strftime('%Y-%m-%d')}")
        print(f"📊 Val until: {val_end_date.strftime('%Y-%m-%d')}")
        
        # Split data
        train_df = merged_df[merged_df['Date'] < train_end_date].copy()
        val_df = merged_df[(merged_df['Date'] >= train_end_date) & (merged_df['Date'] < val_end_date)].copy()
        test_df = merged_df[merged_df['Date'] >= val_end_date].copy()
        
        # Set Date as index and prepare features
        FEATURES = [col for col in merged_df.columns if col != 'Date']
        LABEL_COLS = ['Auc Price']
        
        train_df = train_df.set_index('Date')[FEATURES].copy()
        val_df = val_df.set_index('Date')[FEATURES].copy()
        test_df = test_df.set_index('Date')[FEATURES].copy()
        
        # Create preprocessor and normalize
    ...
    def train_model(self, merged_df):
        ...
        try:
            ...
            # Use custom data preparation if custom date is provided
            if self.custom_date:
                train_df, test_df, val_df, preprocessor = self.prepare_data_with_custom_date(merged_df)
            else:
    ...
    def save_performance_metrics(self, performance_metrics):
        ...
        try:
            ...
            with open(txt_path, 'w') as f:
                ...
                f.write("=" * 50 + "\n\n")
                f.write(f"Generated on: {self.get_current_timestamp().strftime('%Y-%m-%d %H:%M:%S')}\n\n")
    ...
    def generate_summary_report(self, performance_metrics, trade_log_df):
        ...
        try:
            ...
            with open(report_path, 'w') as f:
                ...
                f.write(f"**Generated:** {self.get_current_timestamp().strftime('%Y-%m-%d %H:%M:%S')}\n\n")
    ...
    def generate_pdf_report(self, performance_metrics, trade_log_df):
        ...
        try:
            from scripts.pdf_report_generator import PDFReportGenerator
    ...
    def run_full_pipeline(self):
        ...
        actual_start_time = datetime.now()  # For duration calculation
        print(f"🎯 Starting Carbon Market Analysis Pipeline at {start_time.strftime('%Y-%m-%d %H:%M:%S')}")
        print("=" * 70)
        
        try:
            # Step 1: Load and preprocess data
            # If custom date is provided, load data up to that date
            if self.custom_date:
                # Calculate start date (e.g., 1 year before custom date for training)
                start_date = (self.custom_date - pd.Timedelta(days=365)).strftime('%Y-%m-%d')
                end_date = self.custom_date.strftime('%Y-%m-%d')
                print(f"📅 Loading data from {start_date} to {end_date}")
                merged_df = self.auction_loader.load_auction_data(start_date=start_date, end_date=end_date)
            else:
                merged_df = self.auction_loader.load_auction_data()
            
            # Step 2: Train model
            # Pass custom date to train_model for proper data splitting
            ...
            # Completion summary
            end_time = datetime.now()  # Use actual time for duration
